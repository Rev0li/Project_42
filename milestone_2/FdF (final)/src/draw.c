...
/*   Updated: 2025/03/07 16:54:48 by okientzl         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_coord	project_iso(t_coord pt, int scale)
{
	...
	new.y = (int)((pt.x * scale + pt.y * scale) * sin(ISO_AGL)) - pt.z * scale;
	new.z = 0;
...
t_line_params	assign_struct_line(t_coord p0, t_coord p1, t_transform *trsfrm)
{
	t_line_params	params;

	params.x0 = p0.x + trsfrm->x_off;
	params.y0 = p0.y + trsfrm->y_off;
	params.x1 = p1.x + trsfrm->x_off;
	params.y1 = p1.y + trsfrm->y_off;
...
static void	draw_map_row(t_data *data, int row, t_transform *trfm, int row_len)
{
	int		j;
	int		next_len;
	t_coord	p0;
